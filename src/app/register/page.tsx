...
import adminLogo from "@/app/assets/img/adminLogoWhite.svg"
...
export default function Register() {
  ...
  const handleRegister = async (dataform : UserRegisterForm) => {
    setLoading(true)
  try {
      const data = await userRegister(dataform)

      if (data.status === 409) {
        setLoading(false)
        return showToast("warning", data.message);
      }
      if (data.error) {
        setLoading(false)
    ...
    { loading ? ( 
      ...
      />
        <div className="max-w-md absolute p-8 md:p-10 rounded-2xl shadow-md shadow-dark-blue border border-gray-50  mx-4 md:mx-0">
            <form className=" flex flex-col items-center" noValidate onSubmit={handleSubmit(handleRegister)} >
                <div className="mb-5">
                    <Image src={adminLogo} width={150} height={100} alt="Imagen logo" />
                </div>
                ...
                <div className="relative mt-5">
                    ...
                    <svg className="flex-shrink-0 w-4 h-4 text-gray-100" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    </div>
                </div>
                    {errors.firstName && (
                ...
                <div className="relative mt-7">
                    ...
                    <div className="flex gap-1">
                      <p className="text-xs text-gray-100">¿Ya tienes cuenta?</p>
                      <Link href="/" className="text-xs text-gray-100 font-semibold">Iniciar sesión</Link>
                    </div>
                  </div>
